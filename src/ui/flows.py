...
def connectivity_carousel(
    ...
    """
    Triple-click flow (STRICT gated order):

    Waiting
       ↓ (triple)
    WiFi Screen (ALWAYS)
       ↓ if WiFi OK
    Online Screen (API) (ONLY if WiFi OK)
       ↓ if API OK
    Logging Screen (ONLY if API OK)
    ...
    - If any required thing isn't ON/OK, immediately return to waiting.
    ...
    # Require logging to be enabled/ON in config
    # (This matches your request: "if logging is turned on go to device screen")
    try:
        logging_on = bool(cfg.get("logging_enabled", False))
    except Exception:
        logging_on = False
    ...
    # ------------------------------------------------------------
    # 4) DEVICE SCREEN (only if Logging enabled)
    # ------------------------------------------------------------
    dev_scr = get_screen("device")
    api_info = _fetch_device_info(cfg)  # safe; returns {} offline or on error

    if dev_scr and hasattr(dev_scr, "show_live"):
        try:
            a = dev_scr.show_live(btn=btn, api_info=api_info)
        except Exception:
            a = wait_for_single(btn)
    else:
    ...
    # Any non-single exits to waiting
    if a != "single":
        return _exit(a)
        ...
        try:
            ...
            if name == "temp" and hasattr(scr, "show_live"):
                try:
                    ...
                    scr.show_live(btn=btn, air=air)
                except TypeError:
                    # Fallback: temp screen only wants btn
                    scr.show_live(btn=btn)

                # After temp live screen exits, update the reading once more for summary
                try:
                    reading = air.finish_sampling(log=False)
                except Exception:
                    pass
...
# ============================================================
def time_flow(btn, oled, cfg, wifi, ds3231, get_screen, flush_ms=250, poll_ms=25):
    ...
    if ts is None:
        try:
            ...
            ts = TimeScreen(oled, cfg, wifi_manager=wifi, ds3231=ds3231)
        except Exception:
            ts = None
    ...
    if ts and hasattr(ts, "show_live"):
        try:
            ...
            ts.show_live(btn=btn, max_seconds=0)
            _post_screen_flush(btn, ms=120, poll_ms=poll_ms)
            return
        except Exception:
            pass
