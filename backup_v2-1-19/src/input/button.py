...
class AirBuddyButton:
    ...
    Optional:
      - Drive an LED while the button is physically held down
        (e.g., your button LED wired from GP18 -> resistor -> LED -> GND)
    """

    def __init__(
            self,
            gpio_pin=15,
            click_window_s=0.5,   # tighter feels better for 3/4 clicks
            debounce_ms=50,
            debug_hold_ms=2000,

            # --- LED while held (optional) ---
            led_gpio=18,          # set to None to disable LED driving
            led_active_high=True, # True: GP18 HIGH turns LED on (LED to GND). False if wired to 3V3.
    ):
        ...
        if led_gpio is not None:
            ...
            self._set_led(False)  # default OFF
    ...
    def _set_led(self, on: bool):
        ...
        if self.led_active_high:
            ...
            # active-low wiring (e.g., LED to 3V3, sinking current to GP)
    ...
    def poll_action(self):
        ...
        Also:
          - drives LED ON while the button is physically held down
        """
        now = time.ticks_ms()
        ...
        if level != self._stable_level:
            if time.ticks_diff(now, self._last_change_ms) >= self.debounce_ms:
                ...
                # LED mirrors held state (pressed == 0)
    ...
    def wait_for_action(self):
        ...
        while self._pressed():
            if time.ticks_diff(time.ticks_ms(), press_start) >= self.debug_hold_ms:
                ...
                while self._pressed():
                    time.sleep_ms(10)
                time.sleep_ms(self.debounce_ms)
                self._set_led(False)  # released: LED OFF
                return "debug"
            time.sleep_ms(10)
        ...
        while click_count < 4:
            ...
            time.sleep_ms(self.debounce_ms)
            self._set_led(True)  # pressed again

            self._wait_for_level(1)
            time.sleep_ms(self.debounce_ms)
            self._set_led(False)  # released
    ...
    def reset(self):
        ...
        # Ensure LED reflects current physical state
        self._set_led(lvl == 0)